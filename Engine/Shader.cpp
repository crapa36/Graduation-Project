...
void Shader::CreateGraphicsShader(const wstring& path, ShaderInfo info, ShaderArg arg) {
    ...
    _graphicsPipelineDesc.PrimitiveTopologyType = GetTopologyType(info.primitiveTopology);
...
void Shader::CreateGraphicsShader(ShaderInfo info) {
    D3D12_INPUT_ELEMENT_DESC desc[] =
    {
        { "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
        { "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 12, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0 },
...
void Shader::Update() {
    if (GetShaderType() == SHADER_TYPE::COMPUTE)
        ...
        GRAPHICS_CMD_LIST->IASetPrimitiveTopology(_info.primitiveTopology);
