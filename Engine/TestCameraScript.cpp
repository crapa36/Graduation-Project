...
void TestCameraScript::LateUpdate() {
    ...
    if (dir.Length() > 0.f) {
        ...
        float dotProduct = clamp(look.Dot(dir), -1.0f, 1.0f);
    ...
    distance = clamp(distance - zoomChange, MIN_ZOOM_DISTANCE, MAX_ZOOM_DISTANCE);
...
//    if (rayDirection.LengthSquared() < numeric_limits<float>::epsilon()) {
...
//            float dotProduct = abs(rayDirection.Dot(avoidanceDirection));
//
