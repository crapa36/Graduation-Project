...
void Light::FinalUpdate() {
    ...
    if (static_cast<LIGHT_TYPE>(_lightInfo.lightType) == LIGHT_TYPE::DIRECTIONAL) {
        ...
        shared_ptr<Texture> shadowTex = GResources->Get<Texture>(L"ShadowTarget");
        _volumeMesh = GResources->Get<Mesh>(L"Rectangle");
        _lightMaterial = GResources->Get<Material>(L"DirLight");
        _volumeMesh = GResources->Get<Mesh>(L"Sphere");
        _lightMaterial = GResources->Get<Material>(L"PointLight");
        _volumeMesh = GResources->Get<Mesh>(L"Sphere");
        _lightMaterial = GResources->Get<Material>(L"PointLight");
